...
function EditarPrimeraComunionContent() {
  ...
  const cargarDatos = useCallback(async () => {
    try {
      ...
      if (recordRes.ok) {
        ...
        setFormData({
          numero_identidad_persona: r.numero_identidad_persona,
          ...
          fecha_primera_comunion: r.fecha_primera_comunion.split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/primera-comunion/${id}`, {
        method: 'PUT',
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="form-control">
                <label className="label">
                  <span className="label-text font-medium">Fecha de primera comunión</span>
                </label>
                <input
                  type="date"
...
export default function EditarPrimeraComunionPage() {
  return (
    <ProtectedRoute requiredPermission="canEditSacramentos">
      <EditarPrimeraComunionContent />
    </ProtectedRoute>
  );
}
