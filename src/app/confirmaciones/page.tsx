...
function ConfirmacionesPageContent() {
  ...
  const [registros, setRegistros] = useState<ConfirmacionRecord[]>([]);
  ...
  const cargarRegistros = useCallback(async () => {
    try {
      ...
      logger.error('Error al cargar confirmaciones:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const eliminarRegistro = async (id: string, nombre: string) => {
    ...
    try {
      const response = await fetch(`/api/confirmaciones/${id}`, { method: 'DELETE' });
      if (response.ok) {
        await Swal.fire({
          icon: 'success',
          title: 'Eliminado',
  ...
  const filtered = registros.filter((r) => {
    ...
    const nombre = `${r.confirmado.nombres} ${r.confirmado.apellidos}`.toLowerCase();
    return (
      nombre.includes(term) ||
      r.confirmado.numero_identidad.includes(searchTerm) ||
            ...
            <div className="bg-base-100 rounded-xl shadow-sm border border-base-300 p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl sm:text-3xl font-bold">Confirmaciones</h1>
                    <p className="text-base-content/70 text-sm mt-1">
                      Registro sacramental de confirmaciones
                    </p>
                  </div>
                </div>
                <CreateSacramentoButton
                  label="Nueva Confirmación"
                  onClick={() => router.push('/confirmaciones/nuevo')}
                />
              </div>
            </div>
                    ...
                    {paginated.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="text-center py-12 text-base-content/60">
                          No hay registros de confirmación
                        </td>
                      </tr>
                    ) : (
                      paginated.map((r) => (
                        <tr key={r.id_confirmacion}>
                          <td>
                            <div className="font-medium">
                              {r.confirmado.nombres} {r.confirmado.apellidos}
                            </div>
                            <div className="text-xs text-base-content/60">
                              {r.confirmado.numero_identidad}
                            </div>
                          </td>
                          <td>{formatFecha(r.fecha_confirmacion)}</td>
                          <td>
                          ...
                          <td>
                            <div className="flex justify-end">
                              <SacramentoActionButtons
                                onEdit={() =>
                                  router.push(`/confirmaciones/${r.id_confirmacion}/editar`)
                                }
                                onDelete={() =>
                                  eliminarRegistro(
                                    r.id_confirmacion,
