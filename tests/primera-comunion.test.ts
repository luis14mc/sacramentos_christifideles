...
describe('UPDATE', () => {
  it('propio -> 200', async () => {
    ...
    expect((await update(putReq(validBody({ numero_libro: '9', numero_pagina: '9', numero_registro: '200' })), ctx(r.id_primera_comunion.toString()))).status).toBe(200);
  });
  it('cross-tenant -> 404', async () => {
    const r = await crearDirecto('201'); setSession(cat.parishB);
    expect((await update(putReq(validBody()), ctx(r.id_primera_comunion.toString()))).status).toBe(404);
  });
  it('participante inexistente -> 400', async () => {
    const r = await crearDirecto('202'); setSession(cat.parishA);
    expect((await update(putReq(validBody({ numero_libro: '9', numero_pagina: '9', numero_registro: '202', numero_identidad_madre: 'NOEXISTE' })), ctx(r.id_primera_comunion.toString()))).status).toBe(400);
  });
  it('participante de otra parroquia -> 400', async () => {
    const r = await crearDirecto('203'); setSession(cat.parishA);
    expect((await update(putReq(validBody({ numero_libro: '9', numero_pagina: '9', numero_registro: '203', numero_identidad_padre: PERSONA_B })), ctx(r.id_primera_comunion.toString()))).status).toBe(400);
  });
  it('sacerdote de otra parroquia -> 400', async () => {
    const r = await crearDirecto('204'); setSession(cat.parishA);
    expect((await update(putReq(validBody({ numero_libro: '9', numero_pagina: '9', numero_registro: '204', numero_identidad_sacerdote: SAC_B })), ctx(r.id_primera_comunion.toString()))).status).toBe(400);
  });
  it('colisión registral -> 409', async () => {
    ...
    setSession(cat.parishA);
    expect((await update(putReq(validBody({ numero_libro: '9', numero_pagina: '9', numero_registro: '205' })), ctx(r2.id_primera_comunion.toString()))).status).toBe(409);
  });
});
...
describe('RBAC', () => {
  ...
  it('solo lectura PUT 403', async () => { const r = await crearDirecto('300'); setSession(cat.parishA, 'solo lectura'); expect((await update(putReq(validBody()), ctx(r.id_primera_comunion.toString()))).status).toBe(403); });
});
...
describe('AUDITORÍA', () => {
  ...
  it('update genera bitácora U', async () => {
    ...
    await update(putReq(validBody({ numero_libro: '9', numero_pagina: '9', numero_registro: '400' })), ctx(r.id_primera_comunion.toString()));
