...
import { GET as list, POST as create } from '@/app/api/confirmaciones/route';
import { GET as getOne, PUT as update } from '@/app/api/confirmaciones/[id]/route';
import { setupCatalogo, seedPersona, seedSacerdote, limpiarCatalogo, type Catalogo } from './helpers/sacramentos-fixtures';

let cat: Catalogo;
const OB_A = 'OB-A';
const OB_B = 'OB-B';
const P = { confirmado: 'CF1', madre: 'MD1', padre: 'PD1', madrina: 'MN1', padrino: 'PN1', catequista: 'CT1' };
const PERSONA_B = 'PBc';
let seq = 0;
...
async function crearDirecto(numeroRegistro: string) {
  return prisma.confirmacion.create({
    data: {
      ...
      numero_identidad_confirmado: P.confirmado, numero_identidad_madre: P.madre, numero_identidad_padre: P.padre,
      numero_identidad_madrina: P.madrina, numero_identidad_padrino: P.padrino, numero_identidad_catequista: P.catequista,
      numero_identidad_obispo: OB_A, fecha_confirmacion: new Date('2026-03-01'),
...
describe('CREATE /api/confirmaciones', () => {
  ...
  it('participante de otra parroquia -> 400', async () => { setSession(cat.parishA); expect((await create(makeReq(validBody({ numero_identidad_confirmado: PERSONA_B })))).status).toBe(400); });
  it('obispo inexistente -> 400', async () => { setSession(cat.parishA); expect((await create(makeReq(validBody({ numero_identidad_obispo: 'NOEXISTE' })))).status).toBe(400); });
  it('obispo de otra parroquia -> 400', async () => { setSession(cat.parishA); expect((await create(makeReq(validBody({ numero_identidad_obispo: OB_B })))).status).toBe(400); });
  it('fecha inválida -> 400', async () => { setSession(cat.parishA); expect((await create(makeReq(validBody({ fecha_confirmacion: 'x' })))).status).toBe(400); });
  it('registrales faltantes -> 400', async () => { setSession(cat.parishA); expect((await create(makeReq(validBody({ numero_libro: '' })))).status).toBe(400); });
...
describe('AUDITORÍA', () => {
  it('create genera bitácora C', async () => {
    ...
    expect(await prisma.bitacoraCrud.count({ where: { nombre_tabla: 'confirmacion', accion: 'C' } })).toBe(1);
  });
